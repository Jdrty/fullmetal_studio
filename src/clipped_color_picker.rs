//! color picker (egui 0.31.1)
#![allow(dead_code)] // vendored helpers in sync with upstream

use eframe::egui::{
    epaint, lerp, remap_clamp, Area, DragValue, Frame, Key, Order, Painter, Response, Sense, Ui,
    UiKind, Widget, WidgetInfo, WidgetType,
};
use eframe::egui::widgets::color_picker::Alpha;
use epaint::{
...
fn background_checkers(painter: &Painter, rect: Rect) {
    let rect = rect.shrink(0.5); // avoid the checkers from peeking through the sides
    if !rect.is_positive() {
        return;
    }
...
fn color_button(ui: &mut Ui, color: Color32, open: bool) -> Response {
    ...
    if ui.is_rect_visible(rect) {
        ...
        ui.painter().rect_stroke(
            rect,
            corner_radius,
            (stroke_width, visuals.bg_fill), // using fill for stroke is intentional, because default style is shit and has no border
            StrokeKind::Inside,
        );
    }
...
/// * `x_value` x axis
/// * `y_value` y axis
/// * `color_at` function that dictates how the mix of saturation and value will be displayed in the 2d slider
fn color_slider_2d(
    ui: &mut Ui,
    x_value: &mut f32,
    y_value: &mut f32,
    ...
    if ui.is_rect_visible(rect) {
        ...
        p.add(Shape::mesh(mesh)); // fill

        p.rect_stroke(rect, 0.0, visuals.bg_stroke, StrokeKind::Inside); // outline

        // show where the slider is at:
        let x = lerp(rect.left()..=rect.right(), *x_value);
        let y = lerp(rect.bottom()..=rect.top(), *y_value);
        let picked_color = color_at(*x_value, *y_value);
        p.add(epaint::CircleShape {
            center: pos2(x, y),
...
/// we use a negative alpha for additive colors within this file (ironic)
